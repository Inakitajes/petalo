...
public enum NotchGlassStyle {
    ...
    /// mid-height. Shapes no taller than the solid band come out as a flat
    /// fill at `collapsedOpacity`: the notch passes 1.0 so the collapsed bar
    /// renders as flat black to hide the camera housing, while the pill
    /// passes its tint so the collapsed bar reads as a flat smoked tint.
    public static func scrimStops(
        ...
        collapsedOpacity: Double = 1.0
    ) -> [Stop] {
        let h = max(height, 1)
        let solid = max(solidBandHeight, 0)
        guard solid < h else {
            // The shape is no taller than the solid band. The notch passes
            // 1.0 so the collapsed bar renders as flat black to hide the
            // camera housing; the pill passes its tint so the collapsed bar
            // reads as a flat smoked tint (no cutout to hide).
            return [
                Stop(location: 0, opacity: collapsedOpacity),
                Stop(location: 1, opacity: collapsedOpacity)
            ]
        }
