...
/// Display-selection policy kept outside AppKit so it remains deterministic
/// and behaviorally testable. The app layer supplies display identifiers and
/// current pointer/focused-window geometry.
...
public enum ScreenSelection {
    ...
    /// Display IDs break exact area ties, independent of system report order.
    public static func displayID(
        ...
        for display in displays {
            ...
            if let bestMatch,
               area < bestMatch.area || (area == bestMatch.area && display.id > bestMatch.id) {
                continue
            }
    ...
    /// Resolves the displays that host Petalo. Pointer and focused-window modes
    /// select one display; all-displays retains each connected display.
    public static func selectDisplayIDs(
        ...
        if mode == .allDisplays { return displays.map(\.id) }
    ...
    public static func selectDisplayID(
        ...
        let pointerID = pointerLocation.flatMap { location in
        ...
        switch mode {
        case .pointer: return pointerID ?? focusedID ?? lastID ?? displays.first?.id
        case .focusedWindow: return focusedID ?? pointerID ?? lastID ?? displays.first?.id
        case .allDisplays: return displays.first?.id
        }
    }
    ...
    private static func intersectionArea(_ first: DisplayFrame, _ second: DisplayFrame) -> CGFloat {
        ...
        let width = min(first.minX + first.width, second.minX + second.width) - max(first.minX, second.minX)
        let height = min(first.minY + first.height, second.minY + second.height) - max(first.minY, second.minY)
    ...
    private static func isValid(_ frame: DisplayFrame) -> Bool {
        frame.minX.isFinite && frame.minY.isFinite && frame.width.isFinite && frame.height.isFinite
            ...
            && (frame.minX + frame.width).isFinite && (frame.minY + frame.height).isFinite
    }
}
